...
def get_gaze_frame(heatmaps, landmarks):
    ...
    for i in range(n):
        ...
        for j in range(c):
            ...
            # i-th heatmap's j value
            # if heatmap[j] = 0, 0 else + heatmep[j]
    ...
    def __init__(self, num_layers, high_resolution:bool = True):
        ...
        if high_resolution:
            self.conv1 = nn.Conv2d(1, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
        # high_resolution : + conv2d = 1/2 feature map
        else:
